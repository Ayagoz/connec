...
all_res = Parallel(n_jobs=5)(delayed(main)(new_roi_thick, Y[:,one[0],one[1]]) for one in tuples)


with open('res/result_all_roi2_roi_thick_to_edge', 'wb') as f:
    pickle.dump(np.array(all_res), f)

print('finished thick')
all_res = Parallel(n_jobs=5)(delayed(main)(new_roi_area, Y[:,one[0],one[1]]) for one in tuples)


with open('res/result_all_roi2_roi_area_to_edge', 'wb') as f:
    pickle.dump(np.array(all_res), f)


print('finished area')

all_res = Parallel(n_jobs=5)(delayed(main)(new_roi_vol, Y[:,one[0],one[1]]) for one in tuples)


with open('res/result_all_roi2_roi_vol_to_edge', 'wb') as f:
    pickle.dump(np.array(all_res), f)
...
norm_all_res = Parallel(n_jobs=5)(delayed(main)(norm_new_roi_thick, norm_Y[:,one[0],one[1]]) for one in tuples)

with open('res/result_all_roi2_norm_roi_thick_to_edge', 'wb') as f:
...
norm_all_res = Parallel(n_jobs=5)(delayed(main)(norm_new_roi_area, norm_Y[:,one[0],one[1]]) for one in tuples)

with open('res/result_all_roi2_norm_roi_area_to_edge', 'wb') as f:
    pickle.dump(np.array(norm_all_res), f)



print('finished normed area')
norm_all_res = Parallel(n_jobs=5)(delayed(main)(norm_new_roi_vol, norm_Y[:,one[0],one[1]]) for one in tuples)

with open('res/result_all_roi2_norm_roi_vol_to_edge', 'wb') as f:
    pickle.dump(np.array(norm_all_res), f)
