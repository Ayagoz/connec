...
import community
def mean_roi_thick(thick, label):
    nodes = sorted(np.unique(label))
    mean_thick = []
    for one in nodes:
        if one != 0:
            idx = np.where(label == one)[0]
            mean_thick += [np.median(thick[:,idx], axis = -1)]
    return np.array(mean_thick).T
def area(label, log_jac, mesh_area):
    nodes = sorted(np.unique(label))
    mean_area = []
    for one in nodes:
        if one != 0:
            idx = np.where(label == one)[0]
            mean_area += [np.sum(mesh_area[:,idx] * np.exp(log_jac[:,idx]), axis = -1)]
    return np.array(mean_area).T
def vol(label, log_jac, mesh_area, thick):
    nodes = sorted(np.unique(label))
    mean_vol = []
    for one in nodes:
        if one != 0:
            idx = np.where(label == one)[0]
            mean_vol += [np.sum(mesh_area[:,idx] * np.exp(log_jac[:,idx]) * thick[:,idx], axis = -1)]
    return np.array(mean_vol).T
